...
const About = () => {
  ...
  return (
    <section id="about" className="section-padding bg-dark-800/50 relative">
      <div className="container mx-auto px-4" ref={ref}>
        ...
        >
          <p className="text-primary-400 font-medium mb-2">Get To Know</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="relative w-full max-w-md mx-auto">
              {/* Decorative elements */}
              <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-tr from-primary-500/20 to-secondary-500/20 rounded-2xl transform -rotate-6"></div>
              <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-tr from-secondary-500/20 to-accent-500/20 rounded-2xl transform rotate-3"></div>

              {/* Main image */}
              <div className="relative rounded-2xl overflow-hidden border-4 border-dark-700">
                <img
                  src={myProfileImage}
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
              {stats.map((stat, index) => (
                <motion.div
                  key={stat.label}
                  className="bg-dark-700 rounded-lg shadow-lg flex flex-col items-center text-center py-4"
                  custom={index}
                  initial="hidden"
                  animate={inView ? "visible" : "hidden"}
            ...
            <div>
              ...
              <motion.button
                ...
                className="bg-primary-500 hover:bg-primary-600 text-white px-6 py-3 rounded-md inline-flex items-center gap-2 transition-colors"
      ...
      {showPopup && (
        ...
        >
          <div className="bg-dark-700 border border-primary-500/30 text-white rounded-lg shadow-lg p-4 flex items-start gap-3 max-w-sm">
            <div className="p-2 bg-primary-500/20 rounded-full text-primary-400 flex-shrink-0">
              <Download size={20} />
            </div>
            <div className="flex-grow">
              <h4 className="font-medium mb-1">CV Not Available</h4>
              <p className="text-gray-300 text-sm">The CV is currently not available for download. Please check back later.</p>
            </div>
            <button 
